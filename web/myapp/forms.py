...
class StudentProfileForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['middle_name', 'student_number', 'section', 'phone', 'birthday', 'profile_picture']
        widgets = {
...
class QuizForm(forms.ModelForm):
    class Meta:
        model = Quiz
        fields = ['title', 'description', 'subject', 'due_date', 'time_limit', 'is_published', 'is_active']
        widgets = {
...
class ExamForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'subject': forms.Select(attrs={
                ...
                'required': True,
                'style': 'width: 100%; padding: 15px; font-size: 16px; border: 1px solid #ddd; border-radius: 8px; color: #854bab; margin-bottom: 15px; background-color: white;'
            }),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Ensure we have the latest subjects
        self.fields['subject'].queryset = Subject.objects.all().order_by('name')
        print(f"Form subjects: {[{'id': s.id, 'name': s.name} for s in self.fields['subject'].queryset]}")
